...
class Cart {
  ...
  static getCartGoods (gId, cartIds = '') {
    //4.如果是登录状态,商品数量从cartIds中来,未登录从浏览器中来
  ...
  checkAll () {
    ...
    // console.log(state);
    // console.log(this.getAttribute('all-key'));//是all-key的值
    all('.check-all')[this.getAttribute('all-key')].checked = state;
    // console.log(all('.check-all')[this.getAttribute('all-key')]);
    //2.让所有的商品选中
    //2-1.获取单个商品的复选框
  ...
  static goodsCheck (eleObj) {
    ...
    if (!state) {
      ...
      //2-3.单个商品选中的个数等于全部的数量(len),则全选选中
      if (len == checkCount) {
